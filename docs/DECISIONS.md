...
- **v1.x adapters in-monorepo:** VS Code (`packages/vscode`) and Zed (`packages/zed` + `crates/chorus-zed-helper`) — same wire protocol, host-specific UI only. Zed v1 is joiner/MCP-only (no host share).
