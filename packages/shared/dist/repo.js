 ...
 */
export function normalizeRepoRemote(raw, opts) {
    let s = raw.trim();
    if (!s)
        return "";
    s = s.replace(/\.git$/i, "");
...
function stripRepoRemotePrefix(lower, extraPrefixes) {
    const prefixes = [
...
function applyRepoRemoteRewrites(normalized, rewrites) {
    let out = normalized;
...
export function repoRemotesMatch(a, b, opts) {
