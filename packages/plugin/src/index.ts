...
export default async function chorusPlugin(input: PluginInput) {
  ...
  const publishedUserKeys = new Set<string>();
  const recentUserPayloadAt = new Map<string, number>();
  ...
  function admitPending(
    ...
    userId?: string;
    displayName?: string;
    pendingQueue: ReturnType<PendingQueue["snapshot"]>;
    ...
    else relay.denyUser(resolved.userId);
  ...
  function rememberSet(set: Set<string>, value: string, max = 400): boolean {
  ...
  function publishSessionUserEvent(sid: string, text: string, messageId?: string): boolean {
    if (!sharing || !sid || !text) return false;
    if (MIRROR_LINE.test(text.trim())) return false;
    if (messageId && !rememberSet(publishedUserKeys, `id:${messageId}`)) return false;
    const now = Date.now();
    const last = recentUserPayloadAt.get(text);
    if (last !== undefined && now - last < USER_PAYLOAD_DEDUPE_MS) return false;
    recentUserPayloadAt.set(text, now);
    if (recentUserPayloadAt.size > 400) {
      const first = recentUserPayloadAt.keys().next().value;
      if (first !== undefined) recentUserPayloadAt.delete(first);
    }
  ...
  relay.setInputHandler(async (content, userId, displayName) => {
    ...
    if (MIRROR_LINE.test(trimmed) || isChorusControlText(trimmed)) return;
  ...
  return {
    ...
    event: async (hookInput: {
      ...
      if (!part.messageID || !hostUserMessageIds.has(part.messageID)) return;
      if (!shouldFanOutHostUserText(part.text)) return;
    ...
    "chat.message": async (
      ...
      const text = userTextFromParts(output.parts);
      if (!shouldFanOutHostUserText(text)) return;

      const messageId = output.message?.id ?? chatInput.messageID;
      if (!publishSessionUserEvent(chatInput.sessionID, text, messageId)) return;
    ...
    "experimental.text.complete": async (
      ...
      if (!sharing || !shouldPublishAssistantText(hookOutput.text)) return;
    ...
    tool: {
      ...
      "chorus-approve": {
        ...
        args: {
          userId: z
            ...
            .optional()
            .describe(
              "Queue slot from the pending join list (1, 2, or A) or the full userId. " +
      ...
      "chorus-deny": {
        description:
          "Deny a pending joiner and disconnect them from the relay. " +
          "Pass the queue number printed next to the joiner (1, 2, …) or letter A=1. " +
          "Full userId still works. Omit the id when only one joiner is waiting.",
        args: {
          userId: z
      ...
      "chorus-status": {
        description:
          "Show the current chorus state: whether sharing, joined, who is connected, " +
