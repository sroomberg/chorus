...
export function isChorusControlText(text) {
 ...
 */
export function shouldFanOutHostUserText(text) {
    const trimmed = text.trim();
    if (!trimmed)
        return false;
    if (MIRROR_LINE.test(trimmed) || LABELED_LINE.test(trimmed))
        return false;
    if (isChorusControlText(trimmed))
        return false;
    return true;
}
/** Joiner user text that should be forwarded as collab.input. */
export function shouldForwardJoinerInput(text) {
    const trimmed = text.trim();
    if (!trimmed)
        return false;
    if (MIRROR_LINE.test(trimmed))
        return false;
    if (isChorusControlText(trimmed))
        return false;
    return true;
...
export function shouldPublishAssistantText(text) {
    const trimmed = text.trim();
    if (!trimmed)
        return false;
    if (HOSTING_LOOP_ERROR.test(trimmed))
        return false;
    if (isChorusControlText(trimmed))
        return false;
    return true;
}
export function formatMirroredEvent(event) {
